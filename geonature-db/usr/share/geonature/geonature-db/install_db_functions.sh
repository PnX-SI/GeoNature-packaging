...
function create_database () {
    ...
    export PGPASSWORD=$POSTGRES_PASSWORD;psql -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -f $SCRIPT_PATH/usershub.sql &>> $LOG_PATH/install_db.log
    echo "Insertion des données minimales dans la base de données..."
    export PGPASSWORD=$POSTGRES_PASSWORD;psql -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -f $SCRIPT_PATH/usershub-data.sql &>> $LOG_PATH/install_db.log
    if 
     $ADD_USERSHUB_SAMPLE_DATA ]]
    then
        echo "Insertion des données exemple dans la base de données..."
        export PGPASSWORD=$POSTGRES_PASSWORD;psql -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -f $SCRIPT_PATH/usershub-dataset.sql &>> $LOG_PATH/install_db.log
    fi
}
